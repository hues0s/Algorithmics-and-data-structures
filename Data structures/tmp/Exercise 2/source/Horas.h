...
public:
	...
	bool operator==(Horas const& h) const {
		if (hora == h.getHora() && minuto == h.getMinuto() && segundo == h.getSegundo()) return true;
		return false;
	}
	...
	Horas operator+(Horas const& h) {
		...
		minuto += (segundo + h.getSegundo()) / (LIMITE_SUPERIOR_MINUTOS_SEGUNDOS + 1);
		segundo = (segundo + h.getSegundo()) % (LIMITE_SUPERIOR_MINUTOS_SEGUNDOS + 1);
		
		hora += (minuto + h.getMinuto()) / (LIMITE_SUPERIOR_MINUTOS_SEGUNDOS + 1);
		minuto = (minuto + h.getMinuto()) % (LIMITE_SUPERIOR_MINUTOS_SEGUNDOS + 1);
		
		hora += h.getHora();
		
		if (hora > LIMITE_SUPERIOR_HORAS)
			throw ("La suma de las horas excede del dia actual");
