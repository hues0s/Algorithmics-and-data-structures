...
//Modificamos la operacion insert para que los elementos del array siempre esten ordenados (MAYOR A MENOR)
template <class T>
void set<T>::insert(T e) {
    ...
    if(empty()){
        ...
        if(!contains(e)){
            ...
            //Utilizamos busqueda tradicional, O(n)
            size_t pos = 0;
            while(pos < contador && datos[pos] > e) ++pos;
            
        
            for(size_t i = contador; i > pos; i--) datos[i] = datos[i-1];
            datos[pos] = e;
            ++contador;
        }
    }
...
template <class T>
T set<T>::getMinimumValue() const {
    //if(!empty())
		return datos[contador - 1];
}

template <class T>
void set<T>::eraseMinimumValue() {
	if(!empty())contador--;
}
